...
export const StaggeredMenu = ({
  position = 'right',
  colors = ['#0a1f12', '#132e1a'],
  items = [],
  socialItems = [],
  displaySocials = true,
  displayItemNumbering = true,
  className,
  menuButtonColor = '#6b9f7a',
  openMenuButtonColor = '#e2f0e6',
  ...
  // Hover intent: open on mouseenter with slight delay, close on mouseleave
  ...
  const handleMouseLeaveButton = useCallback(() => {
    clearTimeout(hoverIntentRef.current);
    // Give time to move to the panel
    hoverIntentRef.current = setTimeout(() => {
      if (!openRef.current) return;
      closeMenu();
    }, 400);
  ...
  // Handle menu item click: scroll to section then close
  ...
  return (
    <div
      className={`sm-scope z-[200] fixed top-0 left-0 w-screen h-screen overflow-hidden pointer-events-none`}
    >
      <div
        className={
      ...
      >
        ...
        >
          {(() => {
            const raw = colors && colors.length ? colors.slice(0, 4) : ['#0a1f12', '#132e1a'];
            let arr = [...raw];
            if (arr.length >= 3) {
        ...
        {/* Toggle Button — triggers on hover over MENU text & + icon */}
        <div 
          className="absolute top-0 right-0 pt-[2em] pr-[2em] z-20 pointer-events-none"
        >
          <button
            ref={toggleBtnRef}
            className="sm-toggle relative inline-flex items-center gap-[0.3rem] bg-transparent border-0 cursor-pointer text-[#6b9f7a] font-medium leading-none overflow-visible pointer-events-auto"
        ...
        <aside
          ...
          aria-hidden={!open}
          style={{
            background: 'linear-gradient(135deg, rgba(10,31,18,0.97) 0%, rgba(2,8,7,0.98) 100%)',
              ...
              {items && items.length ? (
                items.map((it, idx) => (
                  <li className="sm-panel-itemWrap relative overflow-hidden leading-none" key={it.label + idx}>
                    <a
                      className="sm-panel-item relative font-display italic cursor-pointer leading-none tracking-[-1px] transition-[color] duration-200 ease-linear inline-block no-underline pr-[1.4em]"
                      href={it.link}
                  ...
                  {socialItems.map((s, i) => (
                    <li key={s.label + i} className="sm-socials-item">
                      <a
                        ...
                        className="sm-socials-link text-[0.95rem] font-medium text-[#6b9f7a] no-underline relative inline-block py-[2px] transition-[color,opacity] duration-300 ease-linear hover:text-[#4ade80]"
                      >
                        {s.label}
                      </a>
                    </li>
...
.sm-scope .staggered-menu-panel { width: clamp(280px, 38vw, 440px); }
...
.sm-scope .sm-socials-link { font-size: 0.95rem; font-weight: 500; color: #6b9f7a; text-decoration: none; position: relative; padding: 2px 0; display: inline-block; transition: color 0.3s ease, opacity 0.3s ease; }
.sm-scope .sm-socials-link:hover { color: #4ade80; }
.sm-scope .sm-panel-list { list-style: none; margin: 0; padding: 0; display: flex; flex-direction: column; gap: 0.5rem; }
.sm-scope .sm-panel-item { position: relative; }
.sm-scope .sm-panel-itemLabel { display: inline-block; will-change: transform; transform-origin: 50% 100%; }
.sm-scope .sm-panel-item:hover { color: #4ade80 !important; }
.sm-scope .sm-panel-list[data-numbering] { counter-reset: smItem; }
.sm-scope .sm-panel-list[data-numbering] .sm-panel-item::after { counter-increment: smItem; content: counter(smItem, decimal-leading-zero); position: absolute; top: 0.1em; right: 1.2em; font-size: 12px; font-weight: 400; font-family: 'DM Mono', monospace; color: var(--sm-accent, #4ade80); letter-spacing: 0; pointer-events: none; user-select: none; opacity: var(--sm-num-opacity, 0); }
